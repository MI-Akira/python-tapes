 ...
 *   IBM_tape.h
 *
 *   (C) Copyright International Business Machines  Corp., 2001-2021
...
struct log_sense10_page {
	unchar page_code;		/* [IN] Log sense page */
	unchar subpage_code;	/* [IN] Log sense subpage */
	unchar reserved[2];
	unsigned short len;     /* [IN] specific allocation length for the data   */
							/* [OUT] number of valid bytes in data            */
							/* (log_page_header size + page_length)           */
...
struct enh_log_sense {
	unchar page_code;       /* [IN] Log sense page */
    unchar subpage_code;    /* [IN] Log sense sub-page */
    unchar page_control;    /* [IN] Page control */
    unchar reserved[5];
    unsigned short len;     /* [IN] specific allocation length for logdatap   */
                            /*      by application */
                            /* [OUT] the length of return data at logdatap    */
                            /*       from driver                              */
    unsigned short parm_pointer; /* [IN] specific parameter number at which   */
                                 /*      the data begins */
...
#define SIOC_ENH_LOG_SENSE	      _IOR('z', 0x58, struct enh_log_sense)
...
struct scsi_path {
	...
	UINT64 primary_fcp_scsi_id;     /* primary FCP id                     */
	UINT64 primary_fcp_lun_id;      /* primary FCP LUN                    */
	UINT64 primary_fcp_ww_name;     /* primary FCP ww name                */
	...
	UINT64 alternate_fcp_scsi_id;   /* alternate FCP SCSI id              */
	UINT64 alternate_fcp_lun_id;    /* alternate FCP LUN                  */
	UINT64 alternate_fcp_ww_name;   /* alternate FCP ww name              */
...
struct device_path_t {
	...
	UINT64 fcp_scsi_id;             /* FCP SCSi id of device              */
	UINT64 fcp_lun_id;              /* FCP logical unit of device         */
	UINT64 fcp_ww_name;             /* FCP world wide name                */
...
/******************************************************************************/
/* Recommended Access Order (rao) IOCTLs                                      */
/******************************************************************************/
...
#define STIOC_QUERY_RAO _IOWR('z', 0x55, struct query_rao_info)
#define STIOC_GENERATE_RAO _IOW('z', 0x56, struct generate_rao)
#define STIOC_RECEIVE_RAO _IOR('z', 0x57, struct receive_rao_list)
...
/******************************************************************************/
/* For preemption through special open                                        */
/******************************************************************************/
#define STIOC_SET_SPDEV _IOR('z', 0x59, struct sp_dev)
...
/******************************************************************************/
/* QUERY/SET ARCHIVE MODE UNTHREAD IOCTLs                                      */
/******************************************************************************/
#define DEVICE_ATTRIBUTES_PAGE 0x30
#define FEATURE_SWITCHES 0x43

#define STIOC_SET_AMU _IOW('z', 0x61, struct archive_mode_unthread)

struct archive_mode_unthread {
    unchar amu_enable;       /* [IN/OUT] 0: amu disabled              */
                             /*          1: amu enabled               */
    unchar reserved[7];
};

#define STIOC_QUERY_AMU _IOR('z', 0x60, struct archive_mode_unthread)

/******************************************************************************/
/* TAPE_LOAD_UNLOAD IOCTL                                      */
/******************************************************************************/
struct tape_load_unload {
    unchar immediate;
    unchar hold;       /* request MAM accessible and volume not for access */
    unchar eot;        /* end of tape, no use and always 0 now */
    unchar retention;  /* archive mode unthread */
    unchar load;       /* perform a load or unload */
    char reserved[11];
};
#define STIOC_LOAD_UNLOAD   _IOW('z', 0x62, struct tape_load_unload)
